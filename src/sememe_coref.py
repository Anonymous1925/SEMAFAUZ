...
#2#[coreference modifications] : array of the replacement pairs using coreference [word, replacement]
#3#[dependency modifications] : array of the replacement pairs using dependency parsing [word, replacement]
#4#[atomic modifications]
#5#nb dictionary: nb words detected by dictionary
#6#nb sememe: words detected by sememes
#7#similarity check : if the mutant passed the check 1 or not 0 or ablation -1
...
def getSentenceReplacements(original_text):
    ...
    if len(dependency_predicator._tokenizer.tokenize(text)) < 3: return None
